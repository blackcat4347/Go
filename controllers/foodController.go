...
func GetFoods() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		result, err := foodCollection.Aggregate(ctx, mongo.Pipeline{match, groupStage, projectStage})
		defer cancel()
		if err != nil {
...
func toFixed(num float64, precision int) float64 {
	output := math.Pow(10, float64(precison))
...
func UpdateFood() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if food.Menu_id != nil {
			err := menuCollection.FineOne(ctx, bson.M{"menu_id": food.Menu.id}).Decode(&menu)
			defer cancel()
			if err != nil {
